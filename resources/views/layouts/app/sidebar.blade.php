    ...
    <body class="portal-shell min-h-screen text-[color:var(--portal-ink)] antialiased">
        ...
        <flux:sidebar sticky collapsible="mobile" class="portal-sidebar border-e border-zinc-200/80 bg-white/85 backdrop-blur-sm">
            ...
            <flux:sidebar.nav>
                ...
                @if ($showStaffNavigation)
                    <flux:sidebar.group :heading="__('Jogathon')" class="grid gap-1 mt-2">
                        @can('enterJogathonPhysicalCollections')
                            <flux:sidebar.item icon="identification" :href="route('teacher.jogathon.cards.index')" :current="request()->routeIs('teacher.jogathon.*')" wire:navigate>
                                {{ __('Kad Jogathon') }}
                            </flux:sidebar.item>
                        @endcan
